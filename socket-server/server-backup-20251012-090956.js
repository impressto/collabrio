...
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    ...
    // Create cached_images table for persistent image storage
    db.run(`
...
const CACHED_IMAGES_DIR = path.join(__dirname, 'cached-images');
...
const getSessionCacheDir = (sessionId) => {
  const sessionDir = path.join(CACHED_IMAGES_DIR, sessionId);
...
// Helper function to cache an image for a session
const cacheImageForSession = async (sessionId, fileData, uploaderUsername) => {
  try {
    ...
    const sessionDir = getSessionCacheDir(sessionId);
    ...
    await cleanupOldCachedImages(sessionId);
...
const cleanupOldCachedImages = async (sessionId) => {
  try {
    // Get all cached images for this session, ordered by upload time (newest first)
    ...
    if (images.length > CACHED_IMAGE_CONFIG.maxImagesPerSession) {
      ...
      for (const image of imagesToDelete) {
        ...
        await new Promise((resolve, reject) => {
          db.run('DELETE FROM cached_images WHERE id = ?', [image.id], (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
...
const getCachedImagesForSession = async (sessionId) => {
  try {
    return await new Promise((resolve, reject) => {
      db.all(`
        ...
        ORDER BY upload_timestamp DESC
        LIMIT ?
      `, [sessionId, CACHED_IMAGE_CONFIG.maxImagesPerSession], (err, rows) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
...
const sendCachedImagesToClient = async (sessionId, socket) => {
  try {
    const cachedImages = await getCachedImagesForSession(sessionId);
...
// Function to clean up cached images for inactive sessions
async function cleanupInactiveCachedImages() {
  try {
    const inactiveThreshold = Date.now() - (7 * 24 * 60 * 60 * 1000); // 7 days ago
    
    // Get cached images from sessions that are older than threshold
    ...
    try {
      const sessionDirs = await fsPromises.readdir(CACHED_IMAGES_DIR);
      for (const sessionDir of sessionDirs) {
        const sessionPath = path.join(CACHED_IMAGES_DIR, sessionDir);
...
// Clean up old sessions and cached images on startup and then every 24 hours
cleanupOldSessions();
cleanupInactiveCachedImages();
setInterval(() => {
  cleanupOldSessions();
  cleanupInactiveCachedImages();
}, 24 * 60 * 60 * 1000);
...
app.post('/upload-file', upload.single('file'), (req, res) => {
  try {
    ...
    // Cache image if it's an image file (async, don't wait for completion)
    console.log(`Checking if file should be cached - MIME type: ${req.file.mimetype}, isImage: ${isImageFile(req.file.mimetype)}`);
    if (isImageFile(req.file.mimetype)) {
      console.log(`Attempting to cache image: ${req.file.originalname}`);
      cacheImageForSession(sessionId, fileData, uploaderUsername).catch(err => {
...
// Endpoint to serve cached images
app.get('/cached-image/:sessionId/:fileId', async (req, res) => {
  try {
    ...
    if (!fs.existsSync(imageInfo.file_path)) {
      ...
      return res.status(404).json({
        status: 'error',
        message: 'Cached image file not found on disk'
      });
    }
    ...
    res.setHeader('Content-Length', imageInfo.file_size);
    res.setHeader('Content-Disposition', `inline; filename="${imageInfo.filename}"`);
...
io.on('connection', (socket) => {
  ...
  socket.on('join-session', ({ sessionId: sid, clientId: cid, userIdentity, schoolAuth }) => {
    ...
    if (sessionDocuments.has(sessionId)) {
      ...
      // Send cached images for this session
      sendCachedImagesToClient(sessionId, socket);
    } else {
      ...
      loadSessionFromDatabase(sessionId)
        .then((documentContent) => {
          ...
          // Send cached images for this session (after document load)
          sendCachedImagesToClient(sessionId, socket);
        })
        .catch((error) => {
          ...
          // Still try to send cached images even if document load failed
          sendCachedImagesToClient(sessionId, socket);
        });
    }
  });
  ...
  socket.on('file-chunk', ({ fileId, chunkIndex, chunkData, isLastChunk }) => {
    ...
    if (fileTransfer.receivedChunks === fileTransfer.totalChunks || isLastChunk) {
      ...
      if (sortedChunks.length === fileTransfer.totalChunks) {
        ...
        // Cache image if it's an image file (async, don't wait for completion)
        console.log(`Checking if file should be cached - MIME type: ${fileTransfer.mimeType}, isImage: ${isImageFile(fileTransfer.mimeType)}`);
        if (isImageFile(fileTransfer.mimeType)) {
          console.log(`Attempting to cache image: ${fileTransfer.filename}`);
          // Create fileData object compatible with cacheImageForSession
          ...
          cacheImageForSession(sessionId, fileDataForCaching, uploaderUsername).catch(err => {
            console.error('Error caching image:', err);
          });
        } else {
